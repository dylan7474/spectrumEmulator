...
Press **Tab** at any time to summon the tape manager popup. The overlay pauses Spectrum key routing while it displays the current deck state, the mounted tape (if any), and the active recorder destination. From the manager you can press **P**, **S**, **W**, or **R** to mirror the play/stop/rewind/record shortcuts (holding **Shift** with **R** continues a recording in append mode), **E** to eject the current tape, and **Tab** or **Esc** to close the window. Hitting **L** opens an inline file prompt: type or paste a `.tap`, `.tzx`, or `.wav` path and press **Return** to load it immediately without restarting the emulator. The prompt is seeded with the last tape path so swapping images only requires editing the filename.
