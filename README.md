...
While a tape source or recording destination is configured, a status panel in the upper border shows the current deck mode and a running counter so you can monitor playback and capture progress without leaving the emulator window.

To capture the MIC output generated by `SAVE`, provide `--save-tap` for a decoded `.tap` container or `--save-wav` for a raw audio
dump. The emulator records the cassette pulses while the ROM routines execute and writes the selected format on exit. If you skip
these flags, the first press of **F8** now arms the deck automatically and stores the capture in `recording.tap` in the current
working directory:
...
When a recording destination is configured (or once the default `recording.tap` file has been created), press **F8** to arm the
virtual deck. Each recording session clears the previous capture and is written out automatically when you stop with **F6** or
**F7** or exit the emulator.
...
- F5 Play, F6 Stop, F7 Rewind, F8 Record (if a tape output has been configured).
