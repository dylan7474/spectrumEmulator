...
To capture the MIC output generated by `SAVE`, provide `--save-tap` with a destination file. The emulator records the cassette
pulses while the ROM routines execute and emits a `.tap` container on exit:
