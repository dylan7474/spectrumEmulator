...
Press **Tab** at any time to summon the tape manager popup. The centered overlay pauses Spectrum key routing and renders a deck-style control panel with the loaded tape, the active recorder destination, and a large digital counter. The illuminated play/stop/rewind/record buttons respond to clicks as well as the **P**, **S**, **W**, and **R** shortcuts (hold **Shift** with **R** to append to an existing WAV). A second row highlights the Load (**L**), Eject (**E**), and Close (**Tab**) actions so the available gestures are always visible, and the shortcut strip along the bottom wraps across two lines so every binding stays within the viewport. Press **L** to open the inline file prompt, type or paste a `.tap`, `.tzx`, or `.wav` path, then hit **Return** to mount it immediately; **Esc** cancels the prompt and **Tab** closes the manager from any mode. The status strip updates after every command so you can confirm deck changes without leaving the overlay.
